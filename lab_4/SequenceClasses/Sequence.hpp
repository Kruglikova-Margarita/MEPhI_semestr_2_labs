...
public:
    ...
    virtual void Connect(const Sequence<T> *addedSequence) noexcept = 0;
    ...
    virtual bool ContainsSubSequence(const Sequence<T> &subSequence) const noexcept = 0;
    virtual bool Contains(T data) const noexcept = 0;
...
public:
    ...
    void Connect(const Sequence<T> *addedSequence) noexcept override {
        size_t thisLen = sequence->GetLength();
        size_t addedLen = addedSequence->GetLength();
        for (int i = 0; i < addedLen; i++) {
            T el = addedSequence->Get(i);
            this->Append(el);
        }
    }
    ...
    bool ContainsSubSequence(const Sequence<T> &subSequence) const noexcept override {
        int thisLen = this->GetLength();
        int subLen = subSequence.GetLength();
        if ((thisLen < subLen) || (thisLen == 0) || (subLen == 0)) return false;
        int count = 0;
        for (int i = 0; i < thisLen; i++) {
            if (((*this)[i] == subSequence[0]) && (thisLen - i >= subLen)) {
                count++;
                for (int j = 1; j < subLen; j++) {
                    if ((*this)[j + i] == subSequence[j]) count++;
                }
            }
            if (count == subLen) return true;
            count = 0;
        }
        return false;
    }

    bool Contains(T data) const noexcept override {
